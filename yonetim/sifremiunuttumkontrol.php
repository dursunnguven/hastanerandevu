			...
			<form class="login100-form validate-form" action="sifremiunuttumkontrol.php" method="post">
				...
				<center>
					<?php
						...
						$sql = "SELECT * FROM kullanicilar WHERE kadi LIKE ? AND mail LIKE ?";
						$params = array($kadi, $mail);
						$options = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
						$sorgu = sqlsrv_query($conn, $sql, $params, $options);

						while($satir = sqlsrv_fetch_array($sorgu, SQLSRV_FETCH_ASSOC)) {
							echo '<h5> Sn. '.$satir['adsoyad'].'<br> </h5>';
							echo '<h5> Şifreniz: <b>'.$satir['sifre'].'</b> <br> </h5>';
						}
						
						if($sorgu === false) {
							echo 'Yanlış giriş yaptınız. <br> Lütfen tekrar deneyin. <br><a href="sifremiunuttum.php">Geri git</a>';
						}
					?>
				</center>
			</form>
